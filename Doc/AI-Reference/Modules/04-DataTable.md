...
│  ├── 第 1 行：标题（首列 #）                                  │
│  ├── 第 2 行：字段名 (Id, Name, Type, Attack, Price)          │
│  ├── 第 3 行：数据类型 (int, string, int, int, int)           │
│  ├── 第 4 行：注释（各列说明，可空）                          │
│  └── 第 5 行起：数据行（主键 Id 在第二列）                    │
│  注意：GF_X 导表要求至少 5 行，否则报 Comment row 错误。       │
│  详见：Doc/AAAGameData-Excel使用说明.md 三、3.4 节。           │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  2. 自动导表                                                  │
